...
for data_name in "${data_names[@]}"; do
  if [ "$data_name" == "amazon" ]; then
    ...
    for mode in "${modes[@]}"; do
      for ((K=1; K<=fwd_maxiter; K++)); do
        for ((i=1; i<=repetitions; i++)); do
          # Generate a random seed
          echo "Running $dir/main_flax.py with --data_name=$data_name --seed=$i (Run $i) --fwd_tol=$fwd_tol --fwd_maxiter=$fwd_maxiter"
          (cd "$dir" && CUDA_VISIBLE_DEVICES=1 python main_flax.py --data_name="$data_name" --seed=$i --fwd_tol=$fwd_tol --fwd_maxiter=$fwd_maxiter)
        done
      done
    done
  fi
